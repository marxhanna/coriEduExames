...
function showSDAs() {
  ...
  fetch("https://app.corihealth.com.br/api/2/cefaleia", options)
...
function getDiagnosticos() {
  ...
  if (values.includes("Nenhum Sinal de Alarme")) {
    ...
    fetch("https://app.corihealth.com.br/api/2/cefaleia/diagnostico", options)
      .then((response) => response.json())
      .then((response) => {
...
function openPopup(event) {
  event.stopPropagation(); // Impede que o clique no botão também selecione o card
...
function getExames() {
  ...
  fetch("https://app.corihealth.com.br/api/2/cefaleia/exame", options)
    .then((response) => response.json())
    .then((response) => {
...
function showExames() {
  ...
  ex = ex.filter(
    (item, index, self) =>
      index ===
      self.findIndex(
        (t) => t[1].CefaleiaExames[0].Exame === item[1].CefaleiaExames[0].Exame
      )
  );
      ...
      <div class="segmento-exame" onclick="abrirModal(${i});">
        <div class="conteudo-exame">
          <div class="imagem-exame">
            <img src="/imgs/exames_icons/Cefaleia/${item[1].CefaleiaExames[0].Exame}.png" />
          </div>
          <span>${item[1].CefaleiaExames[0].Exame}</span>
        </div>

        <div class="ui modal modal-exame-${i}">
          <div class="header">${item[1].CefaleiaExames[0].Exame}</div>
          <div class="image content">
            <img class="image" src="/imgs/exames/Cefaleia/${item[1].CefaleiaExames[0].Exame}.jpg" />
            <div class="description">
              <p>${item[1].CefaleiaExames[0].Descricao}</p>
            </div>
          </div>
        </div>
      </div>`;
